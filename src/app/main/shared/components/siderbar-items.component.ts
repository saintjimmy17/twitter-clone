...
@Component({
  ...
  template: `
    ...
    <div class="flex flex-row items-center" *ngFor="let item of items">
      ...
      class="text-white material-icons relative rounded-full h-14 w-14 flex items-center justify-center p-4 hover:bg-slate-300 hover:bg-opacity-10 cursor-pointer">
        {{item.icon}}
      </a>
    ...
    <!-- Tweet Button -->
    <div class="mt-6 lg:hidden h-14 w-14 rounded-full p-4 flex items-center justify-center bg-sky-500 hover:bg-opactity-80 transition cursor-pointer">
...
export class SiderbarItemsComponent implements OnInit, OnDestroy{
